-- ZOQ's Gallery — Promo Codes + Promotional Banners
--
-- Adds an admin-managed discount-code engine and a homepage promotional
-- banner CMS. Promo codes follow the exact write-lockdown convention already
-- used for orders/order_items (Phase 9): no public read or write policy on
-- promo_codes/promo_code_usages at all — every customer-facing read goes
-- through a SECURITY DEFINER RPC, and the only way usage is ever counted or
-- a discount ever applied is inside create_order() itself, which re-validates
-- everything server-side and locks the promo_codes row (`for update`) so
-- concurrent orders against the same code can't bypass its usage limit —
-- the same race the existing `update products set stock = stock - qty where
-- stock >= qty` guard already closes for inventory.
...
create table promo_codes (
  ...
  -- Always stored/queried uppercased by the app, so uniqueness is naturally
  -- case-insensitive without a citext dependency; the check is a cheap
  -- defense-in-depth backstop in case a row is ever written outside the app.
  ...
  constraint promo_codes_percentage_max_100
...
-- ============================================================================
-- promo_code_usages — audit trail + the source of truth for per-customer
-- usage-limit checks. The historically-accurate discount actually applied to
-- an order lives on orders itself (below); this table exists only so
-- "has this customer already used this code N times" can be counted, and so
-- admins have a usage audit trail independent of the orders list.
-- ============================================================================
...
create index idx_promo_code_usages_promo_id on promo_code_usages(promo_code_id);
create index idx_promo_code_usages_promo_user on promo_code_usages(promo_code_id, user_id);
create index idx_promo_code_usages_promo_email on promo_code_usages(promo_code_id, lower(customer_email));
...
create table promotional_banners (
  ...
  subtitle text,
  promo_text text,
  -- Joined at read time rather than storing the code as text, so a banner
  -- can never drift from the promo code it references (e.g. after a code is
  -- edited or deactivated, the banner always reflects current reality).
...
create index idx_promotional_banners_active_order
  on promotional_banners(is_active, display_order);

create trigger trg_promotional_banners_updated_at
before update on promotional_banners
for each row execute function set_updated_at();

-- ============================================================================
-- orders — promo snapshot columns, written only by create_order() below.
-- Historically accurate even if the promo code is later edited or deleted:
-- the discount actually applied is captured here at order time, never
-- re-derived from the (possibly since-changed) promo_codes row.
-- ============================================================================
alter table orders add column promo_code text;
alter table orders add column promo_code_id uuid references promo_codes(id) on delete set null;
alter table orders add column discount_type text check (discount_type is null or discount_type in ('percentage', 'fixed'));
...
-- The existing admin column-level lockdown (Phase 10:
-- `grant update (status, payment_status) on orders to authenticated`) already
-- makes every column above immutable to any admin update, so no further
-- lockdown is needed here for historical accuracy.
...
-- No public policy at all for promo_codes/promo_code_usages — matches the
-- orders/order_items write-lockdown convention. Customer-facing validation
-- only ever happens through validate_promo_code()/create_order(), both
-- SECURITY DEFINER, never a direct table read.
create policy "Admins can read all promo codes"
  on promo_codes for select to authenticated using (is_admin());
create policy "Admins can insert promo codes"
  on promo_codes for insert to authenticated with check (is_admin());
create policy "Admins can update promo codes"
  on promo_codes for update to authenticated using (is_admin()) with check (is_admin());
create policy "Admins can delete promo codes"
  on promo_codes for delete to authenticated using (is_admin());

create policy "Admins can read all promo code usages"
  on promo_code_usages for select to authenticated using (is_admin());

create policy "Public can read active banners"
  on promotional_banners for select
  using (is_active = true);
create policy "Admins can read all banners"
  on promotional_banners for select to authenticated using (is_admin());
create policy "Admins can insert banners"
  on promotional_banners for insert to authenticated with check (is_admin());
create policy "Admins can update banners"
  on promotional_banners for update to authenticated using (is_admin()) with check (is_admin());
create policy "Admins can delete banners"
  on promotional_banners for delete to authenticated using (is_admin());

-- ============================================================================
-- create_order() — rewritten to add optional promo-code redemption. Every
-- line from the Phase 9 guest-checkout version is unchanged except where
-- noted; the promo block runs after v_subtotal is finalized from validated
-- cart items and before the shipping-cost calculation.
-- ============================================================================
...
declare
  ...
  -- Explicit scalars rather than a `record` — PL/pgSQL raises "record ... is
  -- not assigned yet" when a record variable's fields are dereferenced
  -- before any row was ever selected into it, which would happen on every
  -- order placed without a promo code. Scalars are simply NULL until set.
  v_promo_id uuid;
  v_promo_code text;
...
-- ============================================================================
-- validate_promo_code() — read-only preview used by the checkout "Apply"
-- button. Runs the same checks as create_order()'s promo block (minus the
-- row lock, since nothing is written) but never increments usage_count or
-- inserts a promo_code_usages row — only create_order() ever does that, and
-- only for an order that actually commits. The per-customer check only runs
-- when p_email is supplied (checkout may not have a filled/verified email
-- yet at the moment "Apply" is clicked); this is a UX preview only, so a
-- customer who bypasses it gains nothing — create_order() re-validates
-- everything from scratch and is the sole authority over what's charged.
-- ============================================================================
...
declare
  v_user_id uuid := auth.uid();
  -- Explicit scalars rather than a `record` — see the comment on
  -- create_order()'s declare block for why dereferencing an unassigned
  -- record's fields would raise an error here on every "no such code" check.
  v_promo_id uuid;
  v_promo_code text;
  v_promo_discount_type text;
  v_promo_discount_value integer;
