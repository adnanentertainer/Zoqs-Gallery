-- Guest checkout: a customer can now place an order without an account.
-- orders.user_id becomes optional; a guest order instead gets a random,
-- unguessable guest_token, used only so that same guest can view their own
-- order confirmation afterward via get_guest_order() below. The existing
-- "Users can read own orders" RLS policy (auth.uid() = user_id) correctly
-- denies everyone -- including the guest who placed it -- once user_id is
-- null, so a separate, narrowly-scoped path is needed for that one case.
...
begin
  -- Authentication is no longer required: a null v_user_id means this is a
  -- guest order, which gets its own guest_token instead of a user_id.
  ...
  -- postal_code intentionally not required here (see the earlier migration).
...
grant execute on function create_order(jsonb, text, jsonb, text) to authenticated, anon;
revoke execute on function create_order(jsonb, text, jsonb, text) from public;

-- Lets a guest view their own order confirmation. Requires the exact
-- order_number AND its random guest_token (never guessable), and only ever
-- matches a guest order (user_id is null) -- a real customer's order can
-- never be reached through this function even if a token were somehow
-- leaked or guessed.
create or replace function get_guest_order(p_order_number text, p_guest_token uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
grant execute on function get_guest_order(text, uuid) to authenticated, anon;
revoke execute on function get_guest_order(text, uuid) from public;
