-- Postal code is no longer required at checkout — Pakistani addresses are
-- commonly given without one, and delivery riders navigate by
-- address/phone regardless. The column stays NOT NULL (an empty string is
-- stored when omitted); only the "is this field blank" validation moves.
...
begin
  ...
  -- postal_code intentionally not required here (see migration header).
